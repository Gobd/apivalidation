...
func getRulesForType(t reflect.Type) (any, []*av.FieldRules) {
	inst := reflect.New(t)
	if r, ok := inst.Interface().(av.Ruler); ok {
		return inst.Interface(), r.Rules()
	}
	if r, ok := inst.Interface().(av.ContextRuler); ok {
...
func mapFieldsToTags(fields []*av.FieldRules, structVal reflect.Value) error {
	for i, fr := range fields {
		...
		fields[i].SetTag(strings.Split(sf.Tag.Get("json"), ",")[0])
	}
	return nil
}

// applyRulesToSchema calls Describe on each rule for matching schema properties.
func applyRulesToSchema(fields []*av.FieldRules, schema *openapi3.Schema) error {
	for k, propRef := range schema.Properties {
		for _, f := range fields {
			if f.Tag() != k {
				continue
			}
			for _, rule := range f.RuleList() {
...
func schemaDoc(value any) openapi3gen.SchemaCustomizerFn {
	return func(name string, t reflect.Type, _ reflect.StructTag, schema *openapi3.Schema) error {
		...
		fields = av.ExpandFields(context.Background(), vi, fields)
...
func applyValueRulerSchema(t reflect.Type, name string, schema *openapi3.Schema) error {
	inst := reflect.New(t)
	vr, ok := inst.Interface().(av.ValueRuler)
	if !ok {
		return nil
	}
